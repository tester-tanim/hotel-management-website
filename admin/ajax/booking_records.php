<?php
	...
	if(isset($_POST['get_bookings']))
   {
		...
		$query = "SELECT bo.*, bd.* FROM `booking_order` bo INNER JOIN `booking_details` bd ON bo.booking_id = bd.booking_id WHERE ((bo.booking_status = 'booked' AND bo.arrival = 1) OR (bo.booking_status = 'cancelled' AND bo.refund = 1) OR (bo.booking_status = 'pending')) AND (bo.order_id LIKE ? OR bd.phonenum LIKE ? OR bd.user_name LIKE ?) ORDER BY bo.booking_id DESC";
			...
			if($data['booking_status'] == 'booked')
         {
			...
			$table_data .= "
						<tr>
							...
							<td>
								<button type='button' onclick='download($data[booking_id])' class='btn btn-outline-danger shadow-none'>
		...
		if ($total_rows > $limit)
      {
         ...
         //ceil function float to integer value rounded up to the next highest integer
