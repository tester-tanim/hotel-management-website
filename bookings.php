...
<body class="bg-light">
    ...
    <div class="container">
        <div class="row">
            ...
            <?php
                ...
                $query = "SELECT bo.*, bd.* FROM `booking_order` bo INNER JOIN `booking_details` bd ON bo.booking_id = bd.booking_id WHERE ((bo.booking_status = 'booked') OR (bo.booking_status = 'cancelled') OR (bo.booking_status = 'pending')) AND (bo.user_id=?) ORDER BY bo.booking_id DESC";
                ...
                while($data = mysqli_fetch_assoc($result))
                {
                    ...
                    if($data['booking_status'] == 'booked')
                    {
                        ...
                        if($data['arrival'] == 1)
                        {
                            ...
                            $btn = "<button onclick='cancel_booking($data[booking_id])' type='button' class='btn btn-danger btn-sm shadow-none'>Cencel</button>
                        ";
                        }
    ...
    <script>
        ...
        {
            if(confirm('Are sure to cancel booking?'))
            {
